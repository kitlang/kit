module Kit.Compiler.Passes.IncludeCModules where
  ...
  decomposeCDecl (CDecl spec [] _) =
    let (storageSpec, _, _, typeSpec, _, _) = partitionDeclSpecs spec in
      [("_", storageSpec, typeSpec, [])]
  decomposeCDecl (CDecl spec xs _) =
    let (storageSpec, _, _, typeSpec, _, _) = partitionDeclSpecs spec in
      [(case declr of {Just d -> cDeclrName d; Nothing -> "_"},
      storageSpec,
      typeSpec,
      case declr of {Just d -> cDeclrDerived d; Nothing -> []}
      ) | (declr, _, _) <- xs]
