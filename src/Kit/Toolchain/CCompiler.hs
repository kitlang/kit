...
toolchianSearchPaths x | x == "linux" || x == "darwin" = (toolchainSearchPaths "") ++ ["/etc/kitlang/toolchains"]
...
loadToolchain name = do
  ...
  if exists
    then loadToolchainFile name
      ...
      let paths = [path </> name <.> "env" | path <- toolchainSearchPaths os]
      found <- foldM (\acc path -> case acc of Just x -> return acc; Nothing -> do
                                                                                   exists <- doesFileExist path
                                                                                   return $ if exists then Just path else Nothing) Nothing paths
      case found of
...
loadToolchainFile fp = do
  ...
  overrides <- forM ["CC", "INCLUDE_PATHS", "CFLAGS", "CPPFLAGS", "LDFLAGS"] $ \var -> do value <- lookupEnv var; return $ case value of {Just x -> Just (var, x); Nothing -> Nothing}
  return $ foldl (\toolchain (key, value) -> case key of
                                                      ...
                                                      _ -> toolchain) toolchain (env ++ catMaybes overrides)
...
getCppFlags toolchain = cppFlags toolchain ++ ["-I" ++ dir | dir <- ccIncludePaths toolchain] ++ cFlags toolchain
getLdFlags toolchain = cFlags toolchain ++ ldFlags toolchain

findCcache :: IO (Maybe FilePath)
findCcache = findExecutable "ccache"

invokeCompiler :: Toolchain -> (String -> IO ()) -> Bool -> FilePath -> FilePath -> IO (Either String String)
...
invokeLinker :: Toolchain -> (String -> IO ()) -> Bool -> [FilePath] -> FilePath -> IO FilePath
invokeLinker toolchain logger shared files out = do
  let args = files ++ (getLdFlags toolchain) ++ (if shared then ["-shared"] else []) ++ ["-o" ++ out]
