module Kit.Compiler.Passes.IncludeCModulesSpec where
  ...
  spec = do
    ...
    describe "Parses C headers" $ do
      ...
      forM_ [
          ("Parses var declarations", "var1", VarBinding (TypeBasicType $ BasicTypeInt 16)),
          ("Parses var definitions", "var2", VarBinding (TypeBasicType $ BasicTypeInt 8)),
          ("Parses var definitions with multiple type specifiers", "var3", VarBinding (TypeBasicType $ BasicTypeUint 32)),
          ("Parses struct vars", "struct_var1", VarBinding (TypeStruct ([], "Struct2") [])),
          ("Parses enum vars", "enum_var1", VarBinding (TypeEnum ([], "Enum1") [])),
          ("Parses pointer vars", "pointer_var1", VarBinding (TypePtr (TypeBasicType $ BasicTypeInt 16))),
          ("Parses pointers to pointer vars", "pointer_var2", VarBinding (TypePtr (TypePtr (TypeBasicType $ BasicTypeInt 16)))),
          ("Parses function pointer vars", "pointer_var3", VarBinding (TypePtr (TypeFunction (TypeBasicType $ BasicTypeInt 16) [("arg1", TypeBasicType $ BasicTypeInt 16)] False))),
          ("Parses void functions", "void_func1", FunctionBinding (TypeBasicType $ BasicTypeVoid) [] False),
          ("Parses functions with non-void types", "int_func1", FunctionBinding (TypeBasicType $ BasicTypeInt 16) [] False),
          ("Parses functions with arguments", "func_with_args", FunctionBinding (TypeBasicType $ BasicTypeFloat 32) [
              ("arg1", TypeBasicType $ BasicTypeInt 16),
              ("arg2", TypeBasicType $ BasicTypeUint 64)
            ] False),
          ("Parses functions with struct return value/arguments", "struct_func", FunctionBinding (TypeStruct ([], "Struct1") []) [
              ("a", TypeStruct ([], "Struct2") [])
            ] False),
          ("Parses functions with pointer return value/arguments", "pointer_func", FunctionBinding (TypePtr $ TypeBasicType $ BasicTypeFloat 32) [
